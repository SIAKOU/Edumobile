...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _handleAuthResult(AuthResponse response,
    ...
    if (response.session == null) throw Exception('Authentification échouée.');
    ...
    if (!mounted) return;
    _redirectToDashboardByRole(_defaultSignupRole);
  }
  ...
  Future<void> _signup() async {
    ...
    try {
      ...
      if (user != null) {
        // Insère dans la table users personnalisée
  ...
  Future<void> signInWithGitHub() async {
    ...
    try {
      final AuthResponse response =
          (await _authService.signInWithGitHub()) as AuthResponse;
      final user = response.user;
      if (user != null) {
        await Supabase.instance.client.from('users').insert({
          'id': user.id,
      ...
      await _handleAuthResult(response, provider: 'github');
    } on AuthException catch (e) {
